...
const { BACKLOG_API_KEY, BACKLOG_SPACE_URL, BACKLOG_PROJECT_KEY } = env;
...
export async function fetchMilestoneMap(
  ...
  const res = await axios.get<Milestone[]>(
    `${BACKLOG_SPACE_URL}/api/v2/projects/${BACKLOG_PROJECT_KEY}/versions`,
...
export async function fetchIssueDetail(
  ...
  const res = await axios.get<IssueDetail>(
    `${BACKLOG_SPACE_URL}/api/v2/issues/${issueIdOrKey}`,
    { params: { apiKey: BACKLOG_API_KEY } }
  );
